// Define frequency for each note
#define C3  131
#define Cs3 139
#define D3  147
#define Ds3 156
#define E3  165
#define F3  175
#define Fs3 185
#define G3  196
#define Gs3 208
#define A3  220
#define As3 233
#define B3  247

#define C4  262
#define Cs4 277
#define D4  294
#define Ds4 311
#define E4  330
#define F4  349
#define Fs4 370
#define G4  392
#define Gs4 415
#define A4  440
#define As4 466
#define B4  494

#define C5  523
#define Cs5 554
#define D5  587
#define Ds5 622
#define E5  660
#define F5  698
#define Fs5 740
#define G5  784
#define Gs5 831
#define A5  880
#define As5 932
#define B5  988

#define C6  1047
#define Cs6 1109
#define D6  1175
#define Ds6 1245
#define E6  1319
#define F6  1397
#define Fs6 1480
#define G6  1568
#define Gs6 1661
#define A6  1760
#define As6 1865
#define B6  1976

// Define tempo for the music
#define SIXTEENTH      64
#define EIGHTH         2*SIXTEENTH
#define DOTTED_EIGHTH  3*SIXTEENTH
#define QUARTER        4*SIXTEENTH
#define DOTTED_QUARTER 6*SIXTEENTH
#define HALF           8*SIXTEENTH
#define WHOLE          16*SIXTEENTH

#define BUZZER 9

#define button 13

void setup() 
{ 
  pinMode(BUZZER ,OUTPUT);

  pinMode(button, INPUT_PULLUP);
}
 
void loop() 
{
  if(digitalRead(button) == 0)        // check if Arduino input pin (13) is at LOW level
  {
    while(digitalRead(button) == 0);  // holding the button....

    for(int i = 0; i< 3; i++)
    {
      note(BUZZER, E6, EIGHTH);
      note(BUZZER, D6, EIGHTH);
      note(BUZZER, Fs5, QUARTER);
      note(BUZZER, Gs5, QUARTER);
  
      note(BUZZER, Cs6, EIGHTH);
      note(BUZZER, B5, EIGHTH);
      note(BUZZER, D5, QUARTER);
      note(BUZZER, E5, QUARTER);
  
      note(BUZZER, B5, EIGHTH);
      note(BUZZER, A5, EIGHTH);
      note(BUZZER, Cs5, QUARTER);
      note(BUZZER, E5, QUARTER);
  
      note(BUZZER, A5, HALF + QUARTER);
      delay(1024);
    }
  }
}

void note(unsigned int pin, unsigned int note, unsigned int duration)
{
  tone(pin, note);
  delay(duration);
  noTone(pin);
}
